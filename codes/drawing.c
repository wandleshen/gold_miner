...
void showRank(){
	...
	MovePen(wWidth/2-2.5,wHeight/2+2);
	drawRec(5,-4);
	SetPenColor("black");
	MovePen(wWidth/2-2*fontSize,wHeight/2+2-fontHeight);
	DrawTextString("Rank");
	MovePen(wWidth/2-2.5,wHeight/2+2-2*fontHeight);
	DrawTextString(" username");
	MovePen(wWidth/2-2*fontSize-0.5,wHeight/2+2-2*fontHeight);
	DrawTextString("level");
	MovePen(wWidth/2+0.5,wHeight/2+2-2*fontHeight);
	DrawTextString("score ");
	MovePen(wWidth/2+1.5,wHeight/2+2-2*fontHeight);
	DrawTextString("mode");
	loadRank();
	...
	while(move != NULL){
		...
		MovePen(wWidth/2-2.35,wHeight/2+2-(2*count+4)*fontHeight);
		DrawTextString(move->userName);
		MovePen(wWidth/2-2*fontSize-0.35,wHeight/2+2-(2*count+4)*fontHeight);
		sprintf(buffer,"%d",move->level);
		DrawTextString(buffer);
		MovePen(wWidth/2+0.65,wHeight/2+2-(2*count+4)*fontHeight);
		sprintf(buffer,"%d ",move->score);
		DrawTextString(buffer);
		MovePen(wWidth/2+1.5,wHeight/2+2-(2*count+4)*fontHeight);
		if(move->isDoublePlayer == 1) DrawTextString("Double");
		else DrawTextString("Single");
		count++;
		move = move->next;
	}
